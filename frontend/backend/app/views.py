...
class NewsView(APIView):
    ...
    def get(self, request):
        ...
        # If no search query, try to get top headlines from multiple sources
        if not q:
            # Try to get top headlines from the specified country
            ...
            try:
                ...
                r = py_requests.get(base, params=params, timeout=10)
                ...
                if r.status_code == 200:
                    ...
                    if articles:
                        ...
                        print(f"Found {len(normalized)} top headlines")
                        return Response({"count": len(normalized), "articles": normalized})
                
                # If no articles from top headlines, try general news
                print("No top headlines found, trying general news...")
                base = "https://newsapi.org/v2/everything"
                params = {"q": "news", "sortBy": "publishedAt", "language": "en", "apiKey": api_key, "pageSize": 20}
                
            except Exception as e:
                print(f"Error fetching top headlines: {e}")
                # Fall through to general news search
        else:
            # User provided a search query
            base = "https://newsapi.org/v2/everything"
            params = {"q": q, "sortBy": "publishedAt", "language": "en", "apiKey": api_key, "pageSize": 20}
